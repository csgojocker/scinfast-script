  ...
  $('.notradelink').addClass('hiddenes');
	...
	$('.inform').removeClass('hiddenes');
		...
		*/
		...
		if(getLang() == "ru"){ 	 	if(data.inf1 && _$('inf1')) _$('inf1').innerHTML = getWordNormal(data.inf1,'','',''); } else {	if(data.inf1 && _$('inf1')) _$('inf1').innerHTML = getWordNormal(data.inf1,'','','');  }
  		...
  		socketIO.on('last-winner', function(data){
			...
			var gold = "";
		(data.name.indexOf('scinfast.ru')>-1)?gold='style="color:gold;"':"";
...
var gold = "";
		(data.user.indexOf('scinfast.ru')>-1)?gold='style="color:gold;"':"";
if(getLang() == "ru"){  
 var block					= '<div class="short item_user_steam" id="'+data.steamid+'">';
					block += '<div class="avatar left"><a href="http://'+data.sitename+'/profile.php?steam_id='+data.steamid+'" target="_blank"><img src="'+data.ava+'" alt="" title="" /></a></div>';
						block += '<ul>\
					<li><a class="user_title_link" '+gold+' original-title="Перейти в профиль" href="/profile.php?steam_id='+data.steamid+'" target="_blank">'+data.user+'</a></li>\
		...
		if(getLang() == "ru"){	$('title').text(data.jackpot.toFixed(2) + currency + ' < scinfast.ru - Проверь свою удачу!');
 } else { $('title').text(data.jackpot/curs.toFixed(2) + currency + ' < scinfast.ru - Проверь свою удачу!');
		}
...
$('#items-count-temps').text('0');
			...
			$('title').text('scinfast.ru > Да прибудет с тобой фортуна');
			$('#timer').text('01:30');
		});
...
if(getLang() == "ru") row1	= '	<div class="list">\
		...
		$.ajax({
			...
			success : function(message){
				...
				var gold = "";
		(el.winnername.indexOf('scinfast.ru')>-1)?gold='style="color:gold;"':"";
		...
		$('.loading').addClass('hiddenes');
			...
			$.each(el.items, function(index1, item) {
				...
				itemsHistory += '<div class="south" style="border:1px solid '+item.background_color+'"><a class="history-item-prize" data-image="http://steamcommunity-a.akamaihd.net/economy/image/'+ item.image+'/85fx70f" data-title="'+ item.itemname+'" data-user="'+ item.ava+'" data-price="~ '+ item.cost+'" data-color="'+ item.background_color+'">\
									<img src="http://steamcommunity-a.akamaihd.net/economy/image/'+ item.image+'/64fx64f" alt="image">\
